...
def parse_args():
    ...
    p.add_argument("--output_dir", default="MedVLM-R1")
    p.add_argument("--model_name", default='JZPeterPan/MedVLM-R1')
    return p.parse_args()
...
def main():
    ...
    PROMPT = """
        ...
        Please return the bounding box of the tool: {tool_name} present in image, by taking help from the instruction: {instruction}
        return a formatted json in below format:
        {{
        "bbox_2d": [x1, y1, x2, y2] for the tool present in image,
        "point_2d": [cx, cy] for the tool present in image
        }}
    """
